...
if [ -n "$H3_RAW" ] && curl -fsSL --retry 3 "$H3_RAW/lock.env" -o /tmp/lock.env; then
    ...
    echo "FATAL: no lock.env (set H3_RAW in the template)"; sleep infinity
fi
source "$H3/lock.env"
...
if [ "$(cat "$H3/installed_version" 2>/dev/null)" != "$LOCK_VERSION" ] || [ ! -f "$COMFY/main.py" ]; then
    install_all || { echo "FATAL: install failed — see $LOG"; sleep infinity; }
else
...
MODELS="$MODELS" COMFY="$COMFY" H3="$H3" "$VENV/bin/python" - <<'PY' || { echo "FATAL: model download failed"; sleep infinity; }
